...
const Product = () => {
  ...
  useEffect(() => {
    ...
    let path = itmFind?.custom_select_landing_page === "Landing Page 2" ? "landingSec" : "landing";
                  ...
                  {user ? (
                    <button
                      ...
                      className={`w-full py-2 lg:px-10 md:px-5 px-3 font-semibold text-white rounded ${
                        count > 0 ? "bg-[#f57224] hover:bg-[#7e3003]" : "bg-gray-400 cursor-not-allowed"
                      }`}>
                      Add To Cart
                    </button>
                  ) : (
              ...
              <div className="flex gap-4 bg-white p-2">
                {webItmData
                  ...
                  .map((itm, idx) => {
                    ...
                    return (
                      <Link to={`../product/${itm.item_code}`} key={idx}>
                        <div className="mt-4 hover:border hover:shadow p-2">
                          <img className="lg:w-96" src={`${base_url + itm.website_image}`} alt="" />
